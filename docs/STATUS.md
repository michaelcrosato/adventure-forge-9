...
See STAGE6_BLACKGLASS.md for the prospective contract and acceptance gates. Manager review rejected a broad engine-factory refactor that accepted mutable scenarios under one global build identity; the clock feature is being kept within the existing validated engine. Peer review found inherited-property resource/fact membership (`constructor`) requiring own-property checks. Content review requires an explicit repair result so Nessa's help and emergency damage cannot be misreported through the aggregate risk total.
