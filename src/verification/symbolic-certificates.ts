...
export function assertCurrentOnly(owner: SymbolicModel, root: Root, path: string): void {
  if (owner.bdd.exists(root, owner.nextVariables) !== root) {
    fail(path, "forest root must be current-only");
  }
}

export function subset(owner: SymbolicModel, left: Root, right: Root, path: string): void {
  if (owner.bdd.and(left, owner.bdd.not(right)) !== 0) fail(path, "set contains a state outside its required superset");
}

export function assertAnchors(source: SymbolicModel, target: SymbolicModel, path: string): Anchors {
...
export function assertFresh(source: SymbolicModel, fresh: SymbolicModel, path: string): void {
...
export function regenerateFailureSeeds(owner: SymbolicModel, anchors: Anchors): {
...
export function importedSingle(owner: SymbolicModel, forest: unknown, path: string): Root {
