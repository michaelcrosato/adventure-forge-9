...
function assertSourcePathScopes(
  ...
  manifest: CertificateBundleManifest,
): void {
...
function verifySourceInputs(root: string, manifest: CertificateBundleManifest): void {
...
function verifyDirectory(root: string, manifest: CertificateBundleManifest): { readonly directory: string; readonly identities: ReadonlyMap<string, { readonly dev: number; readonly ino: number }> } {
...
function verifyManifestSnapshot(
  ...
  assert.deepEqual(parseManifest(parseJsonBytes(bytes), limits), manifest, "manifest contents changed during certificate use");
...
function verifyExpectedBindings(manifest: CertificateBundleManifest, expected: CertificateBundleExpectedBindings): void {
...
export function openCertificateBundle(options: OpenCertificateBundleOptions): CertificateBundle {
  ...
  const manifest = parseManifest(parseJsonBytes(manifestBytes), limits);
  ...
  const recheckIntegrity = (): void => {
    verifyManifestSnapshot(root, manifestPath, manifest, limits, manifestHash, manifestBytes.length);
    verifyExpectedBindings(manifest, options.expected);
    verifyManifestContainer(root, manifestPath, forestDirectory);
    verifySourceInputs(root, manifest);
    verifyCatalog(manifest);
