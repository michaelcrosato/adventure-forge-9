...
import {
  ...
  type SymbolicCertificateProgress,
  type SymbolicCertificateVerificationResult,
...
export interface CampaignCertificateAuditResult {
  /** The verified completion root and the exact BDD owner that contains it. */
  readonly verification: SymbolicCertificateVerificationResult;
  readonly report: CampaignCertificateAudit;
}

/** Fresh exact completion plus source-failure coverage and backward closure. */
export function auditCampaignCertificate(
  onProgress?: (event: SymbolicCertificateProgress) => void,
  ...
  const result = verifySymbolicCertificate(model, bundle.manifest.certificate, bundle.loadForest, {
  ...
  assert.equal(result.failureForestId, "failure-union:");
  const loadedForestIds = bundle.loadedForestIds();
  assert.deepEqual(loadedForestIds, ["failure-union:",
