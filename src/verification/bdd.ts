...
// Keep metadata bounded even when callers declare a very large, sparse universe.
const SUPPORT_MASK_VARIABLE_LIMIT = 512;
...
export class Bdd {
  ...
  private readonly variableSupportMasks: readonly bigint[] | undefined;
  private readonly supportMasks: bigint[] | undefined;
  ...
  public constructor(variableCount: number, options: BddOptions = {}) {
    ...
    this.supportMasks = this.variableCount <= SUPPORT_MASK_VARIABLE_LIMIT ? [] : undefined;
    this.variableSupportMasks = this.supportMasks === undefined ? undefined
      : Array.from({ length: this.variableCount }, (_, variable) => 1n << BigInt(variable));
  }
  ...
  public exists(root: Handle, variables: Iterable<number>): Handle {
    ...
    const quantifiedMask = this.variableMask(ordered);
    if (this.supportDisjoint(root, quantifiedMask)) return root;
    const quantifiedKey = ordered.join(",");
    const key = `exists:${root}:${quantifiedKey}`;
    const cached = this.cacheGet(key);
    if (cached !== undefined) return cached;
    const quantified = new Set(ordered);
    const result = this.existsInternal(root, quantified, quantifiedKey, quantifiedMask);
  ...
  /**
   * Simultaneously replace variables with Boolean functions owned by this
   * manager. Replacement functions are inputs, not recursively substituted.
   * ITE reconstruction handles changed variable order and shared inputs.
   */
  public compose(root: Handle, mapping: ReadonlyMap<number, Handle>): Handle {
    this.assertHandle(root);
    if (mapping === null || mapping === undefined || typeof mapping.entries !== "function") {
      throw new TypeError("mapping must be a Map-like object");
    }
    const substitutions = new Map<number, Handle>();
    for (const [variable, replacement] of mapping.entries()) {
      this.assertVariable(variable);
      this.assertHandle(replacement);
      if (substitutions.has(variable)) throw new RangeError("Duplicate composition variable");
      substitutions.set(variable, replacement);
    }
    if (substitutions.size === 0 || root < 2) return root;
    const affectedMask = this.variableMask(substitutions.keys());
    if (this.supportDisjoint(root, affectedMask)) return root;
    const mappingKey = [...substitutions].sort(([a], [b]) => a - b)
      .map(([variable, replacement]) => `${variable}>${replacement}`).join(",");
    const memo = new Map<Handle, Handle>();
    const visit = (source: Handle): Handle => {
      if (source < 2) return source;
      if (this.supportDisjoint(source, affectedMask)) return source;
      const local = memo.get(source);
      if (local !== undefined) return local;
      const key = `compose:${source}:${mappingKey}`;
      const cached = this.cacheGet(key);
      if (cached !== undefined) { memo.set(source, cached); return cached; }
      const node = this.nodeAt(source);
      const replacement = substitutions.get(node.variable);
      let result: Handle;
      if (replacement === FALSE) result = visit(node.low);
      else if (replacement === TRUE) result = visit(node.high);
      else {
        const low = visit(node.low), high = visit(node.high);
        result = replacement === undefined && low === node.low && high === node.high
          ? source
          : this.iteInternal(replacement ?? this.variable(node.variable), high, low);
      }
  ...
  private existsInternal(
    root: Handle, quantified: ReadonlySet<number>, quantifiedKey: string, quantifiedMask: bigint | undefined,
  ): Handle {
    ...
    if (cached !== undefined) return cached;
    const node = this.nodeAt(root);
    const low = this.existsInternal(node.low, quantified, quantifiedKey, quantifiedMask);
    const high = this.existsInternal(node.high, quantified, quantifiedKey, quantifiedMask);
  ...
  private makeNode(variable: number, low: Handle, high: Handle): Handle {
    ...
    if (this.supportMasks !== undefined) {
      const lowSupport = low < 2 ? 0n : this.supportMasks[low - 3]!;
      const highSupport = high < 2 ? 0n : this.supportMasks[high - 3]!;
      this.supportMasks.push(this.variableSupportMasks![variable]! | lowSupport | highSupport);
    }
  ...
  /** Inputs have already passed the public variable/handle validation. */
  private variableMask(variables: Iterable<number>): bigint | undefined {
    if (this.variableSupportMasks === undefined) return undefined;
    let mask = 0n;
    for (const variable of variables) mask |= this.variableSupportMasks[variable]!;
    return mask;
  }

  private supportDisjoint(root: Handle, mask: bigint | undefined): boolean {
    // Reduced ordered nodes have exactly the union of their child supports
    // plus their own variable. A disjoint mask proves the operation is a no-op.
    return mask !== undefined && (root < 2 || (this.supportMasks![root - 3]! & mask) === 0n);
  }
